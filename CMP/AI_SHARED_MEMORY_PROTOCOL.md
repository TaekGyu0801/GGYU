...
- CMP 프로젝트 안에서 **이택규/주수빈 어느 작업자 채팅이든**, 현재 AI가 실제로 확인할 수 있는 의미 있는 연구 진행은 별도 저장 요청 없이 GitHub에 기록한다.
- 같은 프로젝트의 다른 채팅 내용이 현재 대화 맥락으로 제공되었다면, 그 내용도 작업자와 검증 수준을 구분해 기록할 수 있다.
- 다른 비공개 채팅을 실시간으로 직접 읽거나 감시한다고 가정하지 않는다. 현재 AI가 볼 수 없는 별도 채팅은 그 채팅의 AI가 GitHub에 기록해야 다음 AI가 이어받을 수 있다.
- 코드 원문/로그/결과가 실제로 제공되지 않은 경우에는 진행상황만 `OBSERVED` 또는 `PROPOSED`로 기록하고, 코드/결과를 추정 생성하지 않는다.
